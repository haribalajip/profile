...
const HeroSection = (props) => {
  ...
  const dropMessage = () => {
    window.fcWidget ? window.fcWidget.open() : setTimeout(() => window.fcWidget.open(), 1000);
  }
            ...
            <div className="flex items-center flex-grow flex-shrink-0 lg:flex-grow-0">
              <div className="flex items-center justify-between w-full md:w-auto">
                <a href="#">
                  ...
                  <img className="h-8 w-auto sm:h-10 lg:h-14" src="haribalaji.svg"></img>
                </a>
            ...
            <div className="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
              {
                props.config.links.map(item => {
                  return <a href={item.url} key={item.url} className="font-medium text-gray-500 hover:text-gray-900">{item.label}</a>
                })
              }
            </div>
          </nav>
        ...
        <div className={visibilityState.showMobileNav ? "absolute top-0 inset-x-0 p-2 transition transform origin-top-right" : "hidden"}>
          <div className="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {
                props.config.links.map(item => {
                  return <a href={item.url} key={item.url} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50">{item.label}</a>
                })
              }
            </div>
          </div>
          ...
          <div className="sm:text-center lg:text-left">
            ...
            <div className="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
              <div className="rounded-md shadow">
                <div onClick={dropMessage} className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10">
                  Drop a message
                </div>
              </div>
