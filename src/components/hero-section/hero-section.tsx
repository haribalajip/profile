...
const HeroSection: React.FC<{ config: typeof constants.heroSectionConfig }> = (
  props
) => {
  ...
  const dropMessage = () => {
    // @ts-expect-error
    window.fcWidget // @ts-expect-error
      ? window.fcWidget.open() // @ts-expect-error
      : setTimeout(() => window.fcWidget.open(), 1000);
  };
